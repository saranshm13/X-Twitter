...
//         }
//         else console.log("not working uploading"); 
//     })
...
//         console.log(postData.retweetData);
...
//             // checking if replyTo Field is populated or not
//             if( !postData.replyTo._id ) {
//                 return alert("Reply to is not populated wait what");
//             } 
//             else if( !postData.replyTo.postedBy._id ) {
//                 return alert("Posted by is not populated");
//             }

//             // username of the the account on which replied
//             var replyToUsername = postData.replyTo.postedBy.username;
//             // div for showing 'replying'
...
//             }
//             buttons = ` <button class ='pinButton ${pinnedClass}' data-id ="${postData._id}" data-bs-toggle="modal" data-bs-target="#confirmPinModal"><i class='fas fa-thumbtack'></i></button>
//                         <button id="deletePostButton" data-id ="${postData._id}" data-toggle="modal" data-target="#deletePostModal"><i class='fas fa-times'></i></button>`;
//         }
...
$("#userSearchTextBox").keydown( (event) =>  {
    ...
    if(value == "" && (event.which == 8 || event.keyCode ==  8)) {
        // removing user if selected , code 8 is for delete key 
        selectedUsers.pop();
...
$(document).on("click", ".notification.active", (e) => {
    ...
    // storing anchor tag link but not moving ahead till clicked as it would take time to load
    // so store it and move when it we mark it as read
    e.preventDefault();
...
function messageRecieved(newMessage) {
    // searching element using its attributes
